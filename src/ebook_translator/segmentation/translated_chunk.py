...
class TranslatedChunk(Chunk):
    ...
    def __init__(self, chunk: Chunk, store: "Store"):
        ...
        if chunk.head:
            self.head.update(
                (tag_key, translations[tag_key] or "") for tag_key in chunk.head.keys()
            )

        # Ajouter le body avec indices
        if chunk.body:
            self.body.update(
                (tag_key, translations[tag_key] or "") for tag_key in chunk.body.keys()
            )

        # Ajouter le contexte du tail
        if chunk.tail:
            self.tail.update(
                (tag_key, translations[tag_key] or "") for tag_key in chunk.tail.keys()
            )
