"""Tests de `LineIndexedLLMResponse`."""
...
class TestParseValid:
    def test_simple_two_lines(self):
        raw = "<0/>Bonjour\n<1/>Monde\n[=[END]=]"
        m = LineIndexedLLMResponse.model_validate(raw)
    ...
    def test_dict_input_passthrough(self):
        m = LineIndexedLLMResponse.model_validate({"lines": {0: "x", 1: "y"}})
...
class TestParseErrors:
    def test_missing_end_marker_emits_typed_error(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            LineIndexedLLMResponse.model_validate(raw)
...
class TestHelpers:
    def test_line_indices(self):
        m = LineIndexedLLMResponse.model_validate("<2/>a\n<5/>b\n[=[END]=]")
        assert m.line_indices() == {2, 5}

    def test_fragments_at_unknown_raises_keyerror(self):
        m = LineIndexedLLMResponse.model_validate("<0/>x\n[=[END]=]")
...
class TestMerge:
    def test_union_disjoint(self):
        a = LineIndexedLLMResponse.model_validate("<0/>a\n[=[END]=]")
        b = LineIndexedLLMResponse.model_validate("<1/>b\n[=[END]=]")
        merged = a.merge(b)
        assert merged.lines == {0: "a", 1: "b"}

    def test_other_overrides_on_common_indices(self):
        a = LineIndexedLLMResponse.model_validate("<0/>old\n<1/>keep\n[=[END]=]")
        b = LineIndexedLLMResponse.model_validate("<0/>new\n[=[END]=]")
        merged = a.merge(b)
    ...
    def test_does_not_mutate_inputs(self):
        a = LineIndexedLLMResponse.model_validate("<0/>a\n[=[END]=]")
        b = LineIndexedLLMResponse.model_validate("<0/>b\n[=[END]=]")
        _ = a.merge(b)
        assert a.lines == {0: "a"}
        assert b.lines == {0: "b"}
